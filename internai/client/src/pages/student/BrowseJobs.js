...
export default function BrowseJobs() {
  ...
  const fetchJobs = useCallback(async () => {
    setLoading(true);
    try {
      const params = { page, limit: 8, listingKind: 'job' };
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={`${total} job${total !== 1 ? 's' : ''} available`}
      />
      ...
      {loading ? <Spinner /> : jobs.length === 0 ? (
        <EmptyState icon="◇" title="No full-time jobs found" message="Try adjusting your filters or search." />
      ) : (
    ...
    <Card
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14, gap:12 }}>
        <div style={{ display:'flex', gap:12, alignItems:'center' }}>
          <div style={{ width:44, height:44, borderRadius:10, background:'linear-gradient(135deg,rgba(157,125,245,.2),rgba(0,200,248,.15))', border:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:17, fontWeight:700, color:'var(--purple)', flexShrink:0 }}>
            {job.company?.[0]}
          </div>
          <div>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:2 }}>
              <div style={{ fontFamily:'DM Serif Display,serif', fontSize:16, color:'var(--text)' }}>{job.title}</div>
              <span style={{ background:'rgba(157,125,245,.12)', border:'1px solid rgba(157,125,245,.3)', color:'var(--purple)', fontSize:10, fontWeight:700, padding:'1px 8px', borderRadius:20, textTransform:'uppercase', letterSpacing:.5 }}>Full-time</span>
            </div>
      ...
      <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:16 }}>
        {job.tags?.slice(0,4).map(t => (
          <span key={t} style={{ background:'rgba(157,125,245,.08)', border:'1px solid rgba(157,125,245,.2)', color:'var(--purple)', fontSize:11, padding:'2px 9px', borderRadius:12 }}>{t}</span>
        ))}
      </div>
